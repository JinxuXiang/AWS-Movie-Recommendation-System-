...
def lambda_handler(event, context):
    ...
    print("key:", key)
    print("value:", value)
    sql = "SELECT m.movieId, m.movieName, m.year, g.genres FROM movies m LEFT JOIN genres g ON m.movieId = g.movieId WHERE m.{0} = {1}".format(key, value)
    print(sql)
    ...
    for i in range(len(rows)):
        ...
        result["movieName"] = row[1]
        result["year"] = str(row[2])
        result["genres"] = row[3]
